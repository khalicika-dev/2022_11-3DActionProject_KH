...
namespace Util
{
	...
	bool RayIntersectTri(IN Ray WRay, IN Object* Target, OUT Vector3& HitPoint);

	bool RayIntersectTriNear(IN Ray WRay, IN Object* Target, OUT Vector3& HitPoint);

	//수직으로 내다 꽂은 레이만 사용할것
	bool RayIntersectMap(IN Ray WRay, IN Object* Terrain, OUT Vector3& HitPoint);

	int	 GetMyStandMap(Vector3 worldPos);	// 내가 서있는 맵의 인덱스 확인

	Ray MouseToRay(Vector3 Mouse, Camera* Cam);

	uint64_t CalculateFile(string path);

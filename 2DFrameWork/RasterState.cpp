...
RasterState::RasterState()
{
	...
	desc.CullMode = D3D11_CULL_FRONT;
	D3D->GetDevice()->CreateRasterizerState(&desc, &CullFrontFillWireframe);
	desc.CullMode = D3D11_CULL_NONE;
	D3D->GetDevice()->CreateRasterizerState(&desc, &CullNoneFillWireframe);
	desc.CullMode = D3D11_CULL_BACK;
	D3D->GetDevice()->CreateRasterizerState(&desc, &CullBackFillWireframe);
...
void RasterState::Set(D3D11_CULL_MODE cull, D3D11_FILL_MODE fill)
{
	...
	else if (fill == D3D11_FILL_WIREFRAME)
	{
		if (cull == D3D11_CULL_BACK)
			D3D->GetDC()->RSSetState(CullBackFillWireframe);
		else if (cull == D3D11_CULL_FRONT)
			D3D->GetDC()->RSSetState(CullFrontFillWireframe);
		else if (cull == D3D11_CULL_NONE)
			D3D->GetDC()->RSSetState(CullNoneFillWireframe);
	}
}

...
Pop* Pop::Create(string name)
{
	...
	temp->mesh->LoadFile("8.Pop.mesh", false);
...
float Pop::DefaultRot()
{
	return RANDOM->Float(0.0f, PI * 2.0f);
}
...
void Pop::Reset()
{
	...
	for (UINT i = 0; i < particleCount; i++)
	{
		...
		//임의의 회전된 3개축
		Vector3 rot = rotRange();

		//임의의 회전행렬
		Matrix matRot = Matrix::CreateFromYawPitchRoll(rot.y, rot.x, rot.z);
		// v = v * R
		velocity = Vector3::TransformNormal(velocity, matRot * R);

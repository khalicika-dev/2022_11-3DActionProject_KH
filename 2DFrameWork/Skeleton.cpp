...
uint64_t Skeleton::CalculateFile(string file)
{
    return Util::CalculateFile("Contents/Skeleton/" + file);
}
...
void Skeleton::LoadFile(string file, bool b_AddCapacity)
{
    this->file = file;
    BinaryReader in;
    ...
    {
        bonesOffset[i] = in.matrix(b_AddCapacity);
    }
    in.Close();
}

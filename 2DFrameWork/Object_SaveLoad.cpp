...
void Actor::LoadFile(string file)
{
	...
	if ((component = ob->FirstChildElement("Skeleton")) and not parent)
	{
		...
		skeleton->LoadFile(file, RESOURCE->IsItInXmlList(this->file));
	...
	RESOURCE->AddCurrentCapacity(RESOURCE->IsItInXmlList(this->file), Util::CalculateFile(path));
	SafeDelete(doc);
}

uint64_t Actor::CalculateFile(string file)
{
	uint64_t total = 0;
	Xml::XMLDocument* doc = new Xml::XMLDocument();
	string path = "Contents/GameObject/" + file;
	Xml::XMLError result = doc->LoadFile(path.c_str());
	if (result != Xml::XML_SUCCESS) return 0;
	...
	{
		file = component->Attribute("File");
		total += Skeleton::CalculateFile(file);
	}
	if (component = ob->FirstChildElement("Animations"))
	{
		int vecSize = component->IntAttribute("AnimSize");
		for (int i = 0; i < vecSize; i++)
		{
			string number = "File" + to_string(i);
			string aniFile = component->Attribute(number.c_str());
			total += RESOURCE->animations.Calculate(aniFile);
		}
	}
	ObType type = (ObType)ob->IntAttribute("ObType");
	total += Object::CalculateObject(ob, file, type);

	total += Util::CalculateFile(path);
	SafeDelete(doc);
	return total;
}


void Object::SaveObject(Xml::XMLElement* This, Xml::XMLDocument* doc)
{
	...
	if (collider)
	{
		...
		Collider->SetAttribute("Enable", collider->enable);
		Collider->SetAttribute("Visible", collider->visible);
		Collider->SetAttribute("GameType", collider->gameType);
...
void Object::LoadObject(Xml::XMLElement* This)
{
	Xml::XMLElement* component;
	string file;
	...
	if (component = This->FirstChildElement("Collider"))
	{
		...
		collider->enable = component->BoolAttribute("Enable");
		collider->visible = component->BoolAttribute("Visible");
		collider->gameType = component->IntAttribute("GameType");
	...
	{
		Terrain* TerrainOb = dynamic_cast<Terrain*>(this);
		TerrainOb->dijkstra.LoadFile(TerrainOb->file, RESOURCE->IsItInXmlList(TerrainOb->file));
	}
	else if (type == ObType::Light)
	{
		Light* LightOb = dynamic_cast<Light*>(this);
	...
	for (int i = 0; i != size; i++)
	{
		...
		ObType Type = (ObType)ob->IntAttribute("ObType");
		if (Type == ObType::Object)
		{
			Object* temp = Object::Create(childName);
			AddChild(temp);
			temp->LoadObject(ob);
		}
...
uint64_t Object::CalculateObject(Xml::XMLElement* This, string actorFileName, ObType type)
{
	uint64_t total = 0;
	Xml::XMLElement* component;
	string file;
	...
	if (component = This->FirstChildElement("Mesh"))
	{
		...
		{
			file = component->Attribute("File");
			total += RESOURCE->meshes.Calculate(file);
		}
	}
	if (component = This->FirstChildElement("Shader"))
	{
		file = component->Attribute("File");
		total += RESOURCE->shaders.Calculate(file);
	}
	if (component = This->FirstChildElement("Material"))
	{
		file = component->Attribute("File");
		total += RESOURCE->materials.Calculate(file);
	}
	if (component = This->FirstChildElement("Collider"))
	{
		int coltype;
		coltype = component->IntAttribute("ColliderType");
		total += Collider::CalculateType(((ColliderType)coltype));
	}

	if (type == ObType::Terrain)
	{
		total += Dijkstra::CalculateFile(actorFileName);
	}
	...
	for (int i = 0; i != size; i++)
	{
		...
		string childName = ob->Attribute("Name");
		ObType Type = (ObType)ob->IntAttribute("ObType");
		total += CalculateObject(ob, actorFileName, Type);
	}
	return total;
}

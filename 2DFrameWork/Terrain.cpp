...
void Terrain::CreateStaticMember()
{
    ...
    RESOURCE->AddCurrentCapacity(true, Util::CalculateFile(Util::ToString(path)));
}
...
void Terrain::CalculateStaticMember()
{
    RESOURCE->IncreaseXmlList_AddTotalCapacity("Sphere.xml");
    RESOURCE->IncreaseXmlList_AddTotalCapacity("Line.xml");
    RESOURCE->AddTotalCapacity(Util::CalculateFile("Shaders/ComputeShader.hlsl"));
}
...
Terrain* Terrain::Create(string Name, int terrainSize,  float uvScale)
{
    ...
    result->shader = RESOURCE->shaders.Load("5.Terrain.hlsl");
    result->CreateMesh(terrainSize);
    result->material = make_shared<Material>();
    result->material->ambient.w = 1.0f;
    result->material->normalMap = RESOURCE->textures.Load("Terrain/seafloor.dds");
    result->material->diffuse.w = 1.0f;
    result->material->diffuseMap = RESOURCE->textures.Load("Terrain/water.png");
    return result;
}
...
void Terrain::Render()
{
    ...
    if (showNode && VAR->showColliderComponents)
    {
        DEPTH->Set(false);

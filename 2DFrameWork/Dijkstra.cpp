...
void Dijkstra::LoadFile(string file, bool b_AddCapacity)
{
    this->file = file;
    BinaryReader in;
    wstring path = L"Contents/Dijkstra/" + Util::ToWString(file);
    ...
    {
        NodeList.clear();
        UINT Listsize = in.UInt(b_AddCapacity);
        for (UINT i = 0; i < Listsize; i++)
        {
            DiNode temp;
            temp.id = in.UInt(b_AddCapacity);
            temp.pos = in.vector3(b_AddCapacity);
            NodeList[temp.id] = temp;
            UINT NodeListsize = in.UInt(b_AddCapacity);
            for (UINT j = 0; j < NodeListsize; j++)
            {
                int id = in.Int(b_AddCapacity);
                float cost = in.Float(b_AddCapacity);
...
uint64_t Dijkstra::CalculateFile(string file)
{
    return Util::CalculateFile("Contents/Dijkstra/" + file);
}

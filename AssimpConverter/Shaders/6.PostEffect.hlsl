...
float4 CrossBlur(float2 uv)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        color += TextureD[0].Sample(SamplerD, float2(uv.x + divX, uv.y));
        color += TextureD[0].Sample(SamplerD, float2(uv.x - divX, uv.y));
        color += TextureD[0].Sample(SamplerD, float2(uv.x, uv.y + divY));
        color += TextureD[0].Sample(SamplerD, float2(uv.x, uv.y - divY));
    }
...
float4 OctaBlur(float2 uv)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            color += TextureD[0].Sample(SamplerD, float2(x, y));
        }
    }
...
float4 GaussianBlur(float2 uv)
{
    ...
    for (int i = -6; i <= 6; i++)
    {
        ...
        color += weights[6 + i] * TextureD[0].Sample(SamplerD, temp);
...
float4 Mosaic(float2 uv)
{
    ...
    return TextureD[0].Sample(SamplerD, temp);
}
...
float4 RadialBlur(float2 uv)
{
    ...
    for (int i = 0; i < count; i++)
    {
        color += TextureD[0].Sample(SamplerD, uv);
        uv += delta;
    }
...
float4 Outline(float2 uv)
{
    ...
    for (int i = 1; i <= count; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            ...
            float4 albedo = TextureD[0].Sample(SamplerD, uv + temp);
    ...
    if (sum > 0.0f && sum < 1.0f)
        return TextureD[0].Sample(SamplerD, uv) * 0.9f;
    
    return TextureD[0].Sample(SamplerD, uv);
}
float4 Water(float2 uv)
{
    ...
    float4 Color = TextureD[0].Sample(SamplerD, uv);
...
float4 PS(PixelInput input) : SV_TARGET
{
    ...
    if (select == 0)
        BaseColor = TextureD[0].Sample(SamplerD, input.Uv);
    else if (select == 1)
